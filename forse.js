const valueInput = document.querySelector('.value-input');
      ...
      function submitValues() {
        ...
        for (var key in values) {
          if (validFields.includes(key) && (values[key] < 50 || values[key] > 100) && !errorFound) {
            alert('Marks cannot be lower than 50 or higher than 100.');
            errorFound = true;
          } else if ((key === 'g' || key === 'i') && values[key] != 10 && !errorFound) {
            alert('The mark(s) entered is invalid. Make sure that all values are labelled in green');
            errorFound = true;
